...
- **Narrative Quest** (Game): Narrative Therapy externalization and re-authoring tool. 6 steps: externalize problem as character, hear its voice (speech bubbles), spot exceptions (stars), find strengths, rewrite the story, summary. Age-adaptive (Child/Teen/Adult). Storybook aesthetic with warm earth tones. Components: `client/src/components/tools/narrative-quest/`. Files: NarrativeQuest.tsx, WelcomeScreen.tsx, StepWrapper.tsx, ExternalizeStep.tsx, SpeechBubbles.tsx, ExceptionStars.tsx, StrengthsFinder.tsx, RewriteStep.tsx, StorySummary.tsx, quest-data.ts, index.ts.
