...
import { Home, LayoutDashboard, Gamepad2, User, Menu } from "lucide-react";
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-8">
          ...
          <button className="bg-primary text-primary-foreground px-6 py-2.5 rounded-full text-sm font-medium hover:opacity-90 transition-opacity shadow-lg shadow-primary/20">
            Sign In
          </button>
        </div>
      </motion.nav>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-xl border-t border-border/50 pb-safe">
        <div className="flex justify-around items-center h-16 px-2">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.path} href={item.path}>
                <a className="flex flex-col items-center justify-center w-full h-full gap-1">
                  <div className={cn(
